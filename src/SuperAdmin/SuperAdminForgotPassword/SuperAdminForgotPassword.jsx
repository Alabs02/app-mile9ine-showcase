...
const SuperAdminForgotPassword = () => {
  ...
  const forgotPassword = async () => {
    try {
      setIsLoading(true);
      const { data, status, statusText } = await postRequest(`/super_admin/forgot-password`, transformToFormData({ email }));
            ...
            <div className="col-xl-12 mb-3">
              <Formik
                ...
                onSubmit={async (values, { resetForm }) => {
                  try {
                    setIsLoading(true);
                    const { data, status, statusText } = await postRequest(`/super_admin/update-password`, transformToFormData(values));
                    ...
                    if (data) {
                      ...
                      resetForm();
                      setTimeout(() => {
                        history.push(`/super/admin/signin`);
                      }, 4000);
                    }
                  } catch (err) {
                    catchAxiosErrors(err, setIsLoading, null);

import _, { isEmpty } from 'lodash';
import { Fragment } from 'react';
import { useRecoilValue } from 'recoil';
import { withUserBookedRides } from '../../../recoil/userBookedRides';
import { GiBus } from 'react-icons/gi';
import { ImLocation } from 'react-icons/im';
import NoEntity from '../../NoEntity';
import './ViewBookedRidePartial.css';
import { moneyFormat } from '../../../utils';

const ViewBookedRidePartial = () => {

  const bookedRidesArray =  useRecoilValue(withUserBookedRides);
  console.log('Boked Rides:', bookedRidesArray);

  return (
    <Fragment>
      {(!isEmpty(bookedRidesArray))
        ? <div className="row">
            {bookedRidesArray.map((bookedRide) => (
              <div key={_.get(bookedRide, 'id', null)} className="col-md-4 col-sm-12">
                <div className="card booked-card">
                  <div className="card-body">

                    <div className="d-flex justify-content-between align-items-start">
                      <div>
                        <h5>Booking Code</h5>
                        <div className="badge badge-light px-4 font-w800">{_.get(bookedRide, 'booking_code', null)}</div>
                      </div>

                      <div className="badge badge-danger font-weight-bolder">{moneyFormat.to(_.get(bookedRide, 'fare_amount', null))}</div>
                    </div>
                    

                    <div className="d-flex justify-content-between align-items-start mt-2">
                      <div className="grid-center">
                        <GiBus className="text-green mr-2 mb-1" size={25} />
                        <p className="fs-14 m-0 text-uppercase salutation__overline font-w600">Starting Point</p>
                        <div className="badge bg-dark-yellow px-4 mt-2">{_.get(bookedRide, 'bus.park_ride.starting_point')}</div>
                      </div>
                      <div className="grid-center">
                        <ImLocation className="text-green mr-2 mb-1" size={23} />
                        <p className="fs-14 m-0 text-uppercase salutation__overline font-w600">Destination</p>
                        <div className="badge bg-dark-yellow px-4 mt-2">{_.get(bookedRide, 'bus.park_ride.destination')}</div>
                      </div>
                    </div>
                  </div>

                  <div className="booked-card__footer bg-danger-light py-1">
                    {_.get(bookedRide, 'leaving_date', null)}
                  </div>
                </div>
              </div>
            ))}
          </div>
        : <NoEntity 
            title={`No Booked Rides At This Time`}
            copy={``}
            imgUrl={
              <img alt="" src="data:image/svg+xml;base64," />
            }
          />
      }
    </Fragment>
  );
}

export default ViewBookedRidePartial;


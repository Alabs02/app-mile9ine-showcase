import { isEmpty, findIndex } from 'lodash';
import { Fragment, useState, useEffect } from 'react';
import { useRecoilValue, useRecoilState } from 'recoil';
import { Formik, Form, Field, ErrorMessage } from 'formik';
import { object, string } from 'yup';
import parkDriversAtom, { withParkDriversQuery } from '../../../recoil/parkDrivers';
import { TiEdit } from 'react-icons/ti';
import { RiDeleteBin6Line } from 'react-icons/ri';
import { AiOutlineCloseCircle } from 'react-icons/ai';
import { ThreeDots } from 'react-loading-icons';
import { toast } from 'react-toastify';
import NoEntity from '../../NoEntity';
import { catchAxiosErrors, getToken, transformToFormData } from '../../../utils';
import { postRequest } from '../../../utils/axiosClient';
import { TextField } from '../../FormField';
import './DriverList.css';
import 'animate.css';



const initialFormData = (data) => {
  return {
    name: data?.user.name,
    email: data?.user.email,
    address: data?.address,
  };
}

const updateDriverSchema = object().shape({
  name: string()
    .min(4, 'Too Short')
    .max(70, 'Too Long')
    .required('Required'),
  email: string()
    .email('Invalid Email')
    .required('Required'),
  address: string()
    .min(4, 'Too Short')
    .max(100, 'Too Long')
    .required('Required'),
});

export const DriverModalFooter = ({ values, isValid, errors, modalID, driver }) => {

  const [isLoading, setIsLoading] = useState(false);
  const [parkDrivers, setParkDrivers] = useRecoilState(parkDriversAtom);

  useEffect(() => {
    console.log('new values:', values, isValid);
  }, [values]);

  const updateState = (dId) => {
    const index = findIndex(parkDrivers, (o) => o.id === dId);
    const driverData = parkDrivers[index];

    if (index === -1) {
      console.log('no match')
    } else {
      setParkDrivers([
        ...parkDrivers.slice(0, index), 
        driverData, 
        ...parkDrivers.slice(index+1)
      ]);
    }
  }

  const updateParkDriver = async (driverId) => {
    try {
      setIsLoading(true);

      const { data, status, statusText } = await postRequest(`/park_admin/update-park_driver/${driverId}`, transformToFormData(values), {
        headers: { authorization: `Bearer ${await getToken()}` }
      });State

      if (data) {
        console.log(data, status, statusText);
        toast.success(`Updated Successfully!`);
        updateState(driverId);
        setIsLoading(false);
        setTimeout(() => {
          document.getElementById(`${modalID}`).click();
        }, 100);
      }
    } catch (err) {
      catchAxiosErrors(err, setIsLoading, null);
    }
  }

  return (
    <Fragment>
      <div className="mt-3 d-flex justify-content-end align-items-center">
        <button id="close_update_driver_modal" type="button" className="btn btn-danger light mr-3" data-dismiss="modal">Close</button>
        { isLoading
          ? <ThreeDots className="ml-3 animate__animated animate__pulse" height="1.5em" width="3.5em" stroke="#ec3238" /> 
          : <button onClick={() => updateParkDriver(driver?.id)} type="button" disabled={(isEmpty(errors) && isValid) ? false : true} className="btn btn-primary animate__animated animate__pulse">Update</button>
        }
      </div>
    </Fragment>
  );
}

export const DriversTableActions = ({ uid, loaderId, driver }) => {

  const [isLoading, setIsLoading] = useState(false);
  const [parkDrivers, setParkDrivers] = useRecoilState(parkDriversAtom);

  console.log('Driver=>', driver);
  console.log('From Remove Component: ', parkDrivers);

  const removeParkDriver = async (driverId) => {
    try {
      setIsLoading(true);
      const { data, status, statusText } = await postRequest(`/park_admin/delete-park_driver/${driverId}`, null, {
        headers: { authorization: `Bearer ${await getToken()}` }
      });

      if (data) {
        console.log(data, status, statusText);
        setParkDrivers(parkDrivers.filter(driver => driver?.id !== driverId));
        toast.success(`Deleted Driver Successfully!`);
        setIsLoading(false);
      }
    } catch (err) {
      catchAxiosErrors(err, setIsLoading, null);
    }
  }

  return (
    <Fragment>
      <div id={uid} className="d-flex align-items-center">
        <span data-toggle="modal" data-target={`#exampleUpdateModal${uid}`} id={`${uid}${loaderId}`} className="pointer pointer-scale mr-4">
          <i><TiEdit className="text-secondary" size={"25px"} /></i>
        </span>

        {/* Update Modal */}
        <div className="modal fade" id={`exampleUpdateModal${uid}`} name={`${uid}${loaderId}`}>
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title">Update Park Driver</h5>
                <button type="button" className="close" data-dismiss="modal">
                  <AiOutlineCloseCircle />
                </button>
              </div>

              <div className="modal-body">
                <Formik
                  initialValues={initialFormData(driver)}
                  validationSchema={updateDriverSchema}
                >
                  {props => (
                    <Form>
                      <div className="row">
                        <div className="col-sm-12 col-md-12 mb-3">
                          <label htmlFor="name" className="text-label fs-6 m-0">Driver's Name</label>
                          <Field type="text" name="name" as={TextField} placeholder="e.g John Snow" />
                          <ErrorMessage name="name">
                            {msg => <div className="error-msg text-warning">{msg}</div>}
                          </ErrorMessage>
                        </div>

                        <div className="col-sm-12 col-md-12 mb-3">
                          <label htmlFor="email" className="text-label fs-6 m-0">Driver's Email</label>
                          <Field type="email" name="email" as={TextField} placeholder="e.g snow@example.com" />
                          <ErrorMessage name="email">
                            {msg => <div className="error-msg text-warning">{msg}</div>}
                          </ErrorMessage>
                        </div>

                        <div className="col-sm-12 col-md-12 mb-3">
                          <label htmlFor="address" className="text-label fs-6 m-0">Driver's Address</label>
                          <Field type="text" name="address" as={TextField} placeholder="e.g Somewhere st." />
                          <ErrorMessage name="address">
                            {msg => <div className="error-msg text-warning">{msg}</div>}
                          </ErrorMessage>
                        </div>
                      </div>
                      <DriverModalFooter values={props.values} isValid={props.isValid} errors={props.errors} modalID={`exampleUpdateModal${uid}`} driver={driver} />
                    </Form>
                  )}
                </Formik>
              </div>
            </div>
          </div>
        </div>
        {/* Update Modal */}

        { isLoading
          ? <span id={loaderId} className="mr-4">
              <ThreeDots className="animate__animated animate__pulse" fill={"#ec3238"} height={"1rem"} width={"1.8rem"} /> 
            </span>
          : <span onClick={() => removeParkDriver(driver?.id)} className="pointer pointer-scale mr-4">
              <i><RiDeleteBin6Line className="text-primary" size={"25px"} /></i>
            </span>
        }
      </div>
    </Fragment>
  );
}

const DriverList = () => {
  const driverArray = useRecoilValue(withParkDriversQuery);
  const [parkDrivers, setParkDrivers] = useRecoilState(parkDriversAtom);

  console.log('Driver Array:', driverArray);
  console.log('Local State Atom:', parkDrivers);

  return (
    <Fragment>
      {!isEmpty(driverArray)
        ? <div className="row">
            <div className="col-xl-12">
              <div className="table-responsive">
                <table className="table card-table display dataTablesCard rounded-xl">
                  <thead>
                    <tr>
                      <th>
                        <div className="checkbox mr-0 align-self-center">
                          <div className="custom-control custom-checkbox ">
                            <input type="checkbox" className="custom-control-input" id="checkAll" required />
                            <label className="custom-control-label" htmlFor="checkAll" />
                          </div>
                        </div>
                      </th>
                      <th><strong className="text-muted">S/N</strong></th>
                      <th><strong className="text-muted">Driver's Code</strong></th>
                      <th><strong className="text-muted">Driver's Name</strong></th>
                      <th><strong className="text-muted">Driver's Email</strong></th>
                      <th><strong className="text-muted">Driver's Address</strong></th>
                      <th><strong className="text-muted">Actions</strong></th>
                    </tr>
                  </thead>
                  <tbody>
                    {driverArray.map((driver, index) => (
                      <tr key={driver?.id}>
                        <td>
                          <div className="checkbox mr-0 align-self-center">
                            <div className="custom-control custom-checkbox ">
                              <input type="checkbox" className="custom-control-input" id="customCheckBox2" required />
                              <label className="custom-control-label" htmlFor="customCheckBox2" />
                            </div>
                          </div>
                        </td>
                        <td>{index+=1}</td>
                        <td>
                          <div className="badge badge-light">{driver?.driver_code}</div>
                        </td>
                        <td>{driver?.user.name}</td>
                        <td>{driver?.user.email}</td>
                        <td>{driver?.address}</td>
                        <td>
                          <DriversTableActions 
                            uid={`action${driver?.id}`} 
                            loaderId={`btn${index}${driver?.id}`} 
                            driver={driver} 
                          />
                        </td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              </div>
            </div>
          </div>
        : <NoEntity title={"No Drivers At This Time"} copy={"Goto the Drivers page in other to add new drivers."}
            imgUrl={<img alt="no-drivers" src="data:image/svg+xml;base64," />
            }
          />
      }  
    </Fragment>
  );
}

export default DriverList;

...
const fundStaffSchema = object().shape({
  amount: number()
    .min(1, 'Invalid Amount')
    .required('Required')
});

const reFundStaffSchema = object().shape({
  amount: number()
    .min(1, 'Invalid Amount')
    .required('Required')
});

export const RefundWalletModalAction = ({ values, isValid, errors, resetForm, modalId }) => {
  ...
  const retrieveStaffWallet = async () => {
    try {
      setIsLoading(true);
      const { data, status, statusText } = await postRequest(`/park_admin/withdraw-staff-funds`, transformToFormData(values), {
      ...
      if (data) {
        ...
        toast.success('Funds Retrieved Successfully!');
        setIsLoading(false);
        resetForm();
...
export const RefundWalletModal = ({ staffId, staffName }) => {
  return (
    <Fragment>
      <div className="modal fade" id={`refund_staff_modal${staffId}`}>
        <div className="modal-dialog" role="dialog">
          <div className="modal-content">
            <div className="modal-header">
              <h5 className="modal-title">Retrieve Wallet Funds</h5>
            ...
            <div className="modal-body text-left">
              <Formik
                initialValues={initialFormVal(staffId)}
                validationSchema={reFundStaffSchema}
              >
                {props => (
                  <Form>
                    <div className="row">
                      <div className="col-sm-12 mb-3">
                        <label htmlFor="amount" className="text-label m-0 fs-5">Amount</label>
                        <Field name="amount" as={TextField} placeholder="100" />
                    ...
                    <RefundWalletModalAction
                      values={props.values}
                      isValid={props.isValid}
                      errors={props.errors}
                      resetForm={props.resetForm}
                      modalId={`refund_staff_modal${staffId}`}
                    />
                  </Form>
                )}
              </Formik>
...
export const FundWalletModalAction = ({ values, isValid, errors, resetForm }) => {

  const [isLoading, setIsLoading] = useState(false);
  const [parkStaff, setParkStaff] = useRecoilState(parkStaffAtom);
  const [parkDetails, setParkDetails] = useRecoilState(parkAdminAtom);


  const fundStaffWallet = async () => {
    try {
      setIsLoading(true);
      console.log('Values', values);
      const { data, status, statusText } = await postRequest(`/park_admin/fund_staff`, {
        amount: parseInt(_.get(values, 'amount'), 10),
        agent_id: parseInt(_.get(values, 'agent_id'), 10),
      }, {
        headers: { authorization: `Bearer ${await getToken()}` }
      });
      ...
      if (data) {
        console.log(data, status, statusText);
        setParkStaff({});
        setParkDetails(data?.park_admin);
        console.log('Park Wallet', _.get(parkDetails, 'park.park_wallet_amount', null));
        toast.success(`Wallet Funded Successfully!`);
        setIsLoading(false);
        resetForm();
        setTimeout(() => {
          document.getElementById("close_fund_staff_modal").click();
        }, 100);
      }
    } catch (err) {
      catchAxiosErrors(err, setIsLoading, null);
...
export const FundWalletModal = ({ staffId, staffName }) => {
  return (
    <Fragment>
      <div className="modal fade" id={`fund_Staff_modal${staffId}`}>
        <div className="modal-dialog">
          <div className="modal-content">
            <div className="modal-header">
              <h5 className="modal-title">Fund {_.split(staffName, ' ', 1)}'s Wallet</h5>
              <button type="button" className="close" data-dismiss="modal">
                <AiOutlineCloseCircle />
              </button>
            </div>
            ...
            <div className="modal-body text-left">
              <Formik
                initialValues={initialFormValues(staffId)}
                validationSchema={fundStaffSchema}
              >
                {props => (
                  <Form>
                    <div className="row">
                      <div className="col-sm-12 col-md-12 mb-3">
                        <label htmlFor="amount" className="text-label m-0 fs-5">Amount</label>
                        <Field name="amount" type="number" as={TextField} placeholder="e.g 5000.00" />
                        <ErrorMessage name="amount">
                          {msg => <div className="error-msg text-warning">{msg}</div>}
                        </ErrorMessage>
                      </div>
...
const Staff = () => {
  ...
  console.log('Staff:', staffArray)

  const { parkNameSlug } = useParams();

  useEffect(() => {
    setParkStaff(staffArray || []);
  }, [parkNameSlug]);

import { Fragment, useState } from 'react';
import { useRecoilValue, useRecoilState } from 'recoil';
import { Formik, Form, Field, ErrorMessage, FastField } from 'formik';
import { object, string, number } from 'yup';
import { withParkBusesQuery, parkBusesAtom } from '../../../recoil/parkBuses'
import { TiEdit } from 'react-icons/ti';
import { AiOutlineCloseCircle } from 'react-icons/ai';
import { RiDeleteBin6Line } from 'react-icons/ri';
import { isEmpty, findIndex } from 'lodash';
import { ThreeDots } from 'react-loading-icons';
import { toast } from 'react-toastify';
import { catchAxiosErrors, getToken, transformToFormData } from '../../../utils';
import { postRequest } from '../../../utils/axiosClient';
import NoEntity from '../../NoEntity/NoEntity';
import { TextField } from '../../FormField';
import './BusList.css';
import 'animate.css';

const initialFormData = (data) => {
  return {
    plate_number: data?.plate_number,
    type: data?.type,
    capacity: data?.capacity,
    bus_condition: data?.bus_condition,
  };
}

const updateBusSchema = object().shape({
  plate_number: string()
    .min(4, 'Too Short')
    .max(70, 'Too Long')
    .required('Required'),
  type: string()
    .min(3, 'Too short')
    .max(70, 'Too Long')
    .required('Required'),
  capacity: number()
    .min(1, 'Too Short')
    .required('Required'),
  bus_condition: string()
    .required('Required'),
});

export const BusModalFooter = ({ values, isValid, errors, bus }) => {

  const [isLoading, setIsLoading] = useState(false);
  const [parkBuses, setParkBuses] = useRecoilState(parkBusesAtom);

  const updateState = (bId) => {
    const index = findIndex(parkBuses, (o) => o.id === bId);
    const busData = parkBuses[index];

    if (index === -1) {
      console.log('no match')
    } else {
      setParkBuses([
        ...parkBuses.slice(0, index), 
        busData, 
        ...parkBuses.slice(index+1)
      ]);
    }
  }

  const updateParkBus = async (busId) => {
    try {
      setIsLoading(true);
      const { data, status, statusText } = await postRequest(`/park_admin/update-park_bus/${busId}`, transformToFormData(values), {
        headers: { authorization: `Bearer ${await getToken()}` }
      });

      if (data) {
        console.log(data, status, statusText);
        toast.success(`Updated Successfully!`);
        updateState(busId);
        setIsLoading(false);
        setTimeout(() => {
          document.getElementById("close_update_bus_modal").click();
        }, 100);
      }
    } catch (err) {
      catchAxiosErrors(err, setIsLoading, null);
    }
  }

  return (
    <Fragment>
      <div className="mt-3 d-flex justify-content-end align-items-center">
        <button id="close_update_bus_modal" type="button" className="btn btn-danger light mr-3" data-dismiss="modal">Close</button>
        { isLoading
          ? <ThreeDots className="ml-3 animate__animated animate__pulse" height="1.5em" width="3.5em" stroke="#ec3238" /> 
          : <button onClick={() => updateParkBus(bus?.id)} type="button" disabled={(isEmpty(errors) && isValid) ? false : true} className="btn btn-primary animate__animated animate__pulse">Update</button>
        }
      </div>
    </Fragment>
  );
}

export const TableActions = ({ uid, loaderId, bus }) => {

  const [isLoading, setIsLoading] = useState(false);
  const [parkBuses, setParkBuses] = useRecoilState(parkBusesAtom);

  console.log('Bus=> ', bus);
  console.log('From Remove Component: ', parkBuses);
  
  const removeParkBus = async (busId) => {
    try {
      setIsLoading(true);
      const { data, status, statusText } = await postRequest(`/park_admin/delete-park_bus/${busId}`, null, {
        headers: { authorization: `Bearer ${await getToken()}` }
      });

      if (data) {
        console.log(data, status, statusText);
        setParkBuses(parkBuses.filter(bus => bus?.id !== busId));
        toast.success(`Deleted Bus Successfully!`)
        setIsLoading(false);
      }
    } catch (err) {
      catchAxiosErrors(err, setIsLoading, null);
    }
  }

  return (
    <Fragment>
      <div id={uid} className="d-flex align-items-center">
        <span data-toggle="modal" data-target={`#exampleModalCenterBusList${uid}`} id={`${uid}${loaderId}`} className="pointer pointer-scale mr-4">
          <i><TiEdit className="text-secondary" size={"25px"} /></i>
        </span>

        {/* Update Modal */}
        <div className="modal fade" id={`exampleModalCenterBusList${uid}`} name={`${uid}${loaderId}`}>
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title">Update Park Bus</h5>
                <button type="button" className="close" data-dismiss="modal">
                  <AiOutlineCloseCircle />
                </button>
              </div>

              <div className="modal-body">
                <Formik
                  initialValues={initialFormData(bus)}
                  validationSchema={updateBusSchema}
                >
                  {props => (
                    <Form>
                      <div className="row">
                        <div className="col-sm-12 col-md-12 mb-3">
                          <label htmlFor="plate_number" className="text-label m-0 fs-6">Bus Plate Number</label>
                          <Field type="text" name="plate_number" as={TextField} placeholder="e.g AA-BU89" />
                          <ErrorMessage name="plate_number">
                            {msg => <div className="error-msg text-warning">{msg}</div>}
                          </ErrorMessage>
                        </div>

                        <div className="col-sm-12 col-md-12 mb-3">
                          <label htmlFor="type" className="text-label fs-6 m-0">Bus Type</label>
                          <Field type="text" name="type" as={TextField} placeholder="e.g Travel Bus" />
                          <ErrorMessage name="type">
                            {msg => <div className="error-msg text-warning">{msg}</div>}
                          </ErrorMessage>
                        </div>

                        <div className="col-sm-12 col-md-12 mb-3">
                          <label htmlFor="capacity" className="text-label fs-6 m-0">Bus Capacity</label>
                          <Field type="number" name="capacity" as={TextField} placeholder="e.g 50" />
                          <ErrorMessage name="capacity">
                            {msg => <div className="error-msg text-warning">{msg}</div>}
                          </ErrorMessage>
                        </div>

                        <div className="col-sm-12 col-md-12 mb-3">
                          <label htmlFor="bus_conditions" className="text-label fs-6 m-0">Bus Condition</label>
                          <FastField name="bus_condition">
                            {({ field, form, meta }) => (
                              <Fragment>
                                <select className="form-control" {...field}>
                                  <option value="good">Good</option>
                                  <option value="in_repair">Under Repair</option>
                                  <option value="bad">Bad</option>
                                </select>
                              </Fragment>

                            )}
                          </FastField>
                          <ErrorMessage name="bus_condition">
                            {msg => <div className="error-msg text-warning">{msg}</div>}
                          </ErrorMessage>
                        </div>
                      </div>
                      <BusModalFooter values={props.values} isValid={props.isValid} errors={props.errors} modalID={`#exampleModalCenterBusList${uid}`} bus={bus} />
                    </Form>
                  )}
                </Formik>
              </div>
            </div>
          </div>
        </div>
        {/* Update Modal */}

        { isLoading
          ? <span id={loaderId} className="mr-4">
              <ThreeDots className="animate__animated animate__pulse" fill={"#ec3238"} height={"1rem"} width={"1.8rem"} /> 
            </span>
          : <span onClick={() => removeParkBus(bus?.id)} className="pointer pointer-scale mr-4">
              <i><RiDeleteBin6Line className="text-primary" size={"25px"} /></i>
            </span>
        }
      </div>
    </Fragment>
  );
}

export const BusList = () => {
  const busArray = useRecoilValue(withParkBusesQuery);
  const [parkBuses, setParkBuses] = useRecoilState(parkBusesAtom);
  
  console.log('Bus Array: ', busArray);
  console.log('Local State Atom:', parkBuses);

  return (
    <Fragment>
      {!isEmpty(busArray)
        ? <div className="row">
            <div className="col-xl-12">
              <div className="table-responsive">
                <table className="table card-table display dataTablesCard rounded-xl">
                  <thead>
                    <tr>
                      <th>
                        <div className="checkbox mr-0 align-self-center">
                          <div className="custom-control custom-checkbox ">
                            <input type="checkbox" className="custom-control-input" id="checkAll" required />
                            <label className="custom-control-label" htmlFor="checkAll" />
                          </div>
                        </div>
                      </th>
                      <th><strong className="text-muted">S/N</strong></th>
                      <th><strong className="text-muted">Bus Type</strong></th>
                      <th><strong className="text-muted">Bus Plate Number</strong></th>
                      <th><strong className="text-muted">Bus Capacity</strong></th>
                      <th><strong className="text-muted">Condition</strong></th>
                      <th><strong className="text-muted">Actions</strong></th>
                    </tr>
                  </thead>
                  <tbody>
                    {busArray.map((bus, index) => (
                      <tr key={bus?.id}>
                        <td>
                          <div className="checkbox mr-0 align-self-center">
                            <div className="custom-control custom-checkbox ">
                              <input type="checkbox" className="custom-control-input" id="customCheckBox2" required />
                              <label className="custom-control-label" htmlFor="customCheckBox2" />
                            </div>
                          </div>
                        </td>
                        <td>{index+=1}</td>
                        <td>{bus?.type}</td>
                        <td>{bus?.plate_number}</td>
                        <td>
                          <div className="badge badge-primary">{bus?.capacity}</div>
                        </td>
                        <td>
                          {(bus.bus_condition === 'good')
                            ? <div className="badge badge-success">Good</div>
                            : (bus.bus_condition === 'in_repair')
                            ? <div className="badge bg-dark-yellow">Under Repairs</div>
                            : <div className="badge badge-danger">Bad</div>
                          }
                        </td>
                        <td>
                          <TableActions uid={`actions${bus?.id}`} loaderId={`btn${index}${bus?.id}`} bus={bus} />
                        </td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              </div>
            </div>
          </div>
        : <NoEntity title={"No Buses At This Time"} copy={"Goto the Buses page in other to add new buses."}
            imgUrl={<img alt="no-buses" src="data:image/svg+xml;base64," />
            }
          />
      }
    </Fragment>
  );
}

export default BusList;

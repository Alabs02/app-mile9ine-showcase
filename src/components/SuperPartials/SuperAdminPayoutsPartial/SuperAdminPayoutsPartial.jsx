import { Fragment } from 'react';
import { useRecoilValue } from 'recoil';
import { withSuperAdminPayoutsQuery } from '../../../recoil/Super/superAdminGetPayouts';
import _, { isEmpty } from 'lodash';
import { ImDownload } from 'react-icons/im';
import { moneyFormat } from '../../../utils';
import NoEntity from '../../NoEntity';

const SuperAdminPayoutsPartial = () => {

  const payoutsArray = useRecoilValue(withSuperAdminPayoutsQuery);
  console.log(payoutsArray);

  return (
    <Fragment>
      {(!isEmpty(payoutsArray))
        ? <div className="row">
            <div className="col-xl-12 mb-3">
              <div className="card">
                <div className="card-body w-100">
                  <input type="search" className="form-control px-4" placeholder="Search Payouts" />
                </div>
              </div>
            </div>

            <div className="col-xl-12">
              <div className="d-flex w-100 justify-content-end mb-2">
                <button type="button" className="btn btn-sm bg-dark-yellow">
                  <ImDownload className="mr-2" />
                  Download Payouts
                </button>
              </div>
              <div className="table-responsive">
                <table className="table table-sm card-table display dataTablesCard rounded-xl">
                  <thead>
                    <tr>
                      <th>
                        <div className="checkbox mr-0 align-self-center">
                          <div className="custom-control custom-checkbox ">
                            <input type="checkbox" className="custom-control-input" id="checkAll" required />
                            <label className="custom-control-label" htmlFor="checkAll" />
                          </div>
                        </div>
                      </th>
                      <th><strong className="text-muted">S/N</strong></th>
                      <th><strong className="text-muted">Payout ID</strong></th>
                      <th><strong className="text-muted">Bank Name</strong></th>
                      <th><strong className="text-muted">Account Number</strong></th>
                      <th><strong className="text-muted">Beneficiary Name</strong></th>
                      <th><strong className="text-muted">Amount</strong></th>
                      <th><strong className="text-muted">Reference</strong></th>
                      <th><strong className="text-muted">Status</strong></th>
                      <th><strong className="text-muted">Approval Status</strong></th>
                    </tr>
                  </thead>
                  <tbody>
                    {payoutsArray.map((payout, index) => (
                      <tr key={_.get(payout, 'id', null)}>
                        <td>
                          <div className="checkbox mr-0 align-self-center">
                            <div className="custom-control custom-checkbox ">
                              <input type="checkbox" className="custom-control-input" id="customCheckBox2" required />
                              <label className="custom-control-label" htmlFor="customCheckBox2" />
                            </div>
                          </div>
                        </td>
                        <td>{index+=1}</td>
                        <td>{_.get(payout, 'payout_id', null)}</td>
                        <td>{_.get(payout, 'bank_name', null)}</td>
                        <td>{_.get(payout, 'account_number', null)}</td>
                        <td>{_.get(payout, 'beneficiary_name', null)}</td>
                        <td>
                          <div className="badge badge-light"><span className="mr-2">NGN</span> {moneyFormat.to(_.get(payout, 'amount', null))}</div>
                        </td>
                        <td>{_.get(payout, 'reference', null)}</td>
                        <td>
                          <div className="badge bg-dark-yellow">{_.get(payout, 'status', null)}</div>
                        </td>
                        <td>
                          {(_.get(payout, 'is_approved', null) === 1)
                            ? <div className="badge badge-sucess">Approved</div>
                            : <div className="badge badge-danger">Not Approved</div>
                          }
                        </td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              </div>
            </div>
          </div>
        : <NoEntity 
            title={`No Payouts At This Time`}
            copy={`Once you've made any payout there will appear here!`}
            imgUrl={
              <img alt="" src="data:image/svg+xml;base64," />
            }
          />
      }
        
    </Fragment>
  );
}

export default SuperAdminPayoutsPartial;

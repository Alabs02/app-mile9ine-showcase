...
const ParkAdminForgotPassword = () => {
  ...
  const forgotPassword = async () => {
    try {
      setIsLoading(true);
      const { data, status, statusText } = await postRequest(`/park_admin/forgot-password`, transformToFormData({ email }));
            ...
            <div className="col-xl-12 mb-3">
              <Formik
                ...
                onSubmit={async (values, { resetForm }) => {
                  try {
                    setIsLoading(true);
                    const { data, status, statusText } = await postRequest(`/park_admin/update-password`, transformToFormData(values));
                    ...
                    if (data) {
                      ...
                      resetForm();
                      setTimeout(() => {
                        history.push(`/park/admin/signin`);
                      }, 4000);
                    }
                  } catch (err) {
                    catchAxiosErrors(err, setIsLoading, null);
